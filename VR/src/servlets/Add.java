	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) {
		...
		if (request.getParameter("action").equals("addAppointment")) {
			...
			if(request.getParameter("participants").isBlank() || 
				...
				try {
					request.setAttribute("pagetitle", "Alles kaputt");
					request.setAttribute("message", "<h1>Du hast alles kaputt gemacht!</h1><br> Anfrage unvollst�ndig");
					request.getRequestDispatcher("/templates/failInfoRepresentation.ftl").forward(
							request, response);
				} catch (ServletException | IOException e) {
					request.setAttribute("errormessage",
				...
				System.out.println("Anfrage unvollst�ndig");
				return;
			}
			try {
		...
		try {
			...
			request.setAttribute("message", "Termin Erfolgreich hinzugef�gt");
